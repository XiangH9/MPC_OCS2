...
KeyboardInput::KeyboardInput() : Node("keyboard_input_node") {
    ...
    timer_ = create_wall_timer(std::chrono::microseconds(20), std::bind(&KeyboardInput::timer_callback, this));
...
void KeyboardInput::timer_callback() {
    if (kbhit()) {
        ...
        if (just_published_ && reset_count_ > 0) {
            reset_count_ -= 1;
            if (reset_count_ == 0 && inputs_.command != 0) {
                inputs_.command = 0;
            }
        }
    }
