...
namespace unitree_guide_controller {
    ...
    public:
        ...
        controller_interface::InterfaceConfiguration command_interface_configuration() const override;//命令接口

        controller_interface::InterfaceConfiguration state_interface_configuration() const override;//状态接口

        controller_interface::return_type update(
            const rclcpp::Time &time, const rclcpp::Duration &period) override;// 每个控制周期真正该做的事情

        controller_interface::CallbackReturn on_init() override;// 参数声明 基础配置

        controller_interface::CallbackReturn on_configure(
            const rclcpp_lifecycle::State &previous_state) override;// 订阅建立、运行依赖对象建立

        controller_interface::CallbackReturn on_activate(
            const rclcpp_lifecycle::State &previous_state) override;//真正接上state/command interfaces 让FSM 跑起来
